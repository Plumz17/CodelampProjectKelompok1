...
# ── Wave Data ──────────────────────────────────────────────────
# Each wave: { "enemies": [{ "scene": PackedScene, "count": int }], "interval": float }
...
func start_next_wave() -> void:
	...
	var wave_data: Dictionary = waves[_current_wave_index]
	_build_spawn_queue(wave_data)

	var interval: float = wave_data.get("interval", 1.5)
